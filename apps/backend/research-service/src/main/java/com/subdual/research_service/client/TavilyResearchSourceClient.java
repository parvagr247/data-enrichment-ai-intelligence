...
public class TavilyResearchSourceClient implements SourceDiscoveryProvider {

    private static final Logger log = LoggerFactory.getLogger(TavilyResearchSourceClient.class);
    ...
    public List<DiscoveredSource> discoverSources(String query, int maxResults) {
        ...
        try {
            ...
            log.error("Tavily search provider returned HTTP {}: {}", ex.getStatusCode(), ex.getResponseBodyAsString());
            throw new ExternalServiceException("Search provider unavailable; upstream service returned " + ex.getStatusCode().value(), ex);
        } catch (Exception ex) {
    ...
    private String classifySourceType(String url) {
        ...
        if (lower.contains("docs.") || lower.contains("/docs") || lower.contains("/reference")) {
